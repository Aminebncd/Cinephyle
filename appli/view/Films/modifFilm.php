...
<div class="container modifForm">
    ...
    <form action="index.php?action=modifFilm&id=<?= $id?>" method="POST">
        <div class="mb-3">
            <label for="idFilm" class="form-label titreForm">Film à modifier : <strong>
            <?= $film['titre'] ?>
                </strong></label>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" name="titre" id="titreFilm" value="<?= $film['titre'] ?>" required>
        </div>
        <div class="mb-3">
            <label for="dateSortieFilm" class="form-label">Date de sortie en France :</label>
            <input type="date" class="form-control" name="dateSortieFrance" id="dateSortieFilm" value="<?= $film['date_sortie_france'] ?>" required>
        </div>
        <div class="mb-3">
            <label for="dureeFilm" class="form-label">Durée (en minutes) :</label>
            <input type="number" class="form-control" name="duree" id="dureeFilm" value="<?= $film['duree'] ?>" required>
        </div>
        <div class="mb-3">
            <label for="resumeFilm" class="form-label">Résumé :</label>
            <textarea class="form-control" name="resume" id="resumeFilm" rows="3" required><?= $film['resume'] ?></textarea>
        </div>
        <div class="mb-3">
            <label for="noteFilm" class="form-label">Note (de 1 à 10) :</label>
            <input type="number" class="form-control" name="note" id="noteFilm" min="0" max="10" step="0.5" value="<?= $film['note'] ?>" required>
        </div>
        <div class="mb-3">
            <label for="afficheFilm" class="form-label">URL de l'affiche :</label>
            <input type="url" class="form-control" name="affiche" id="afficheFilm" value="<?= $film['affiche'] ?>" required>
        </div>
        ...
        <div class="mb-3">
            ...
            <select class="form-control" name="idRealisateur" id="idRealisateurFilm" required>
                <?php foreach ($realisateurs as $realisateur) : ?>
                    <option value="<?= $realisateur['id_real'] ?>" <?= ($film['id_real'] == $realisateur['id_real']) ? 'selected' : '' ?>>
